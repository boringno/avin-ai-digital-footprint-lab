...
function isExplicitCreateRequest(text: string) {
  ...
  return (
    /(?:重新|重跑|從頭).{0,10}(?:預約|約診)(?:流程)?/u.test(text) ||
...
function isBookingDecline(text: string) {
  return (
    /(?:先|暫時|目前|現在|還)?不(?:想|要|用|需要|考慮|急著)?(?:先)?(?:預約|字約|約診|約)(?:了|喔|哦)?$/u.test(text) ||
    /(?:先不(?:預約|字約)|暫時不預約|目前不預約|還不預約|不急著預約|先不要約|先不用約)/u.test(text) ||
    /(?:先了解(?:就好|看看)?|暫時先了解)/u.test(text) ||
    /^(?:今天)?(?:先)?(?:測|問|了解|聊)?到這(?:裡)?(?:就)?好(?:了)?$/u.test(text) ||
    /^(?:今天)?先這樣(?:就)?好(?:了)?$/u.test(text) ||
    /^(?:先|暫時)(?:不用|不要)(?:繼續)?(?:填|提供)(?:預約)?資料(?:了)?$/u.test(text)
  );
}
