...
export function containsPriceOrCampaignClaim(text: string) {
  return PRICE_OR_CAMPAIGN_PATTERN.test(text) ||
    UNAPPROVED_PROMOTION_CLAIM_PATTERN.test(text);
}
...
export function constrainMedicalAiReply(text: string, _footer: string, options: AiReplyConstraintOptions = {}) {
  ...
  if (
    !normalized ||
    (medical &&
      (containsPriceOrCampaignClaim(contentWithoutApprovedTreatmentNames) ||
