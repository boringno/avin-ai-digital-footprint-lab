...
async function loadRuntimeOverlayWithSingleRetry(
  ...
  let first: RuntimeContentOverlay;
  try {
    first = await loader(input);
  } catch {
    // The runtime snapshot client is capped at 1.5 seconds per request. One
    // fresh attempt is allowed for a transient transport failure; any second
    // failure remains fail-closed and the total DB wait stays near 3 seconds.
    return loader(input);
  }
