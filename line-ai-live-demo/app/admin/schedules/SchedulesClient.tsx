...
async function makePreview(source: File) {
  ...
  try {
    for (const maxWidth of [1280, 960, 720]) {
      ...
      if (blob && blob.size <= 1024 * 1024) return new File([blob], `${source.name.replace(/\.[^.]+$/, "")}-preview.jpg`, { type: "image/jpeg" });
    }
  } finally {
    bitmap.close();
  }
  throw new Error("無法產生小於 1 MB 的預覽縮圖，請改用較小的原始圖片。");
}
...
export function SchedulesClient({ canEdit, canUseWorkbench, initialMonths }: { canEdit: boolean; canUseWorkbench: boolean; initialMonths: AdminScheduleVersion[] }) {
  ...
  async function submitDraft() {
    ...
    if (!csv) return setError("請上傳當月結構化 CSV。");
    setBusy("create");
    setError("");
    setNotice("");
    try {
      ...
      setPreviews({});
      setPreviewUrls({});
      setNotice("當月四館班表與 CSV 已建立草稿；確認無誤後即可一鍵發布。");
    } catch (caught) {
  ...
  async function act(versionId: string, action: "disable" | "publish") {
    ...
    setNotice("");
    try {
      const response = await fetch("/api/admin/schedules", { body: JSON.stringify({ action, version_id: versionId }), headers: { "content-type": "application/json" }, method: "PATCH" });
      const body = (await response.json()) as { error?: string; months?: AdminScheduleVersion[]; ok?: boolean };
      if (!response.ok || !body.ok) throw new Error(body.error ?? "班表操作失敗。");
      setMonths(body.months ?? []);
      setNotice(action === "publish" ? "該月四館班表與 CSV 已同步發布。系統只會主動傳送當月已發布的班表。" : "該月份已停用，之後不會再主動傳送。已送出的 LINE 圖片無法撤回。");
    } catch (caught) {
  ...
  return <main style={pageStyle}><section style={containerStyle}>
    <header style={headerStyle}><div><p style={eyebrowStyle}>診所營運</p><h1 style={{ margin: "4px 0" }}>門診班表管理</h1><p style={subtleStyle}>一個月份一次發布四館圖片與結構化 CSV。只有當月已發布班表會由 LINE 主動傳送。</p></div><nav style={navStyle}><a href="/admin/content" style={linkStyle}>內容管理</a><a href="/admin/reports" style={linkStyle}>月報</a>{canUseWorkbench ? <a href="/admin/workbench" style={linkStyle}>客服工作台</a> : null}</nav></header>
    {error ? <p style={errorStyle}>{error}</p> : null}{notice ? <p style={noticeStyle}>{notice}</p> : null}
    {canEdit ? <section style={panelStyle}><h2 style={headingStyle}>建立月版草稿</h2><p style={subtleStyle}>請上傳同一月份的四館原圖與 CSV。系統會在此裝置自動產生 LINE 用預覽縮圖；原圖限 JPEG/PNG、10 MB 內，縮圖限 1 MB 內。</p><div style={formGridStyle}><label style={labelStyle}>月份<input onChange={(event) => setSourceMonth(event.target.value)} style={inputStyle} type="month" value={sourceMonth} /></label><label style={labelStyle}>變更原因<input onChange={(event) => setChangeReason(event.target.value)} placeholder="例如：7 月最新門診班表" style={inputStyle} value={changeReason} /></label><label style={labelStyle}>結構化 CSV<input accept=".csv,text/csv" onChange={(event) => setCsv(event.target.files?.[0] ?? null)} style={inputStyle} type="file" /></label></div><p style={subtleStyle}>CSV 欄位：doctor_name、branch、schedule_date、time_slot；可選 source_month、status、notes。四館名稱須為高雄館、台中館、桃園館、林口館。</p><div style={branchGridStyle}>{SCHEDULE_BRANCHES.map((branch) => <label key={branch} style={uploadStyle}><strong>{branch}</strong><input accept="image/jpeg,image/png" onChange={(event) => void selectOriginal(branch, event.target.files?.[0] ?? null)} type="file" /><span style={subtleStyle}>{originals[branch]?.name ?? "尚未選擇原始班表圖"}</span>{previewUrls[branch] ? <img alt={`${branch}預覽縮圖`} src={previewUrls[branch]} style={previewStyle} /> : null}</label>)}</div><button disabled={Boolean(busy)} onClick={() => void submitDraft()} style={primaryButtonStyle} type="button">{busy === "create" ? "建立中..." : "建立草稿"}</button></section> : <section style={panelStyle}>您可以檢視已建立的月版班表，但沒有上傳、發布或停用權限。</section>}
    <section style={panelStyle}><h2 style={headingStyle}>版本紀錄</h2><p style={subtleStyle}>停用後只停止未來主動傳送；LINE 已送出的圖片不會撤回。</p><div style={{ display: "grid", gap: 14 }}>{months.length === 0 ? <p style={subtleStyle}>尚未建立任何月版班表。</p> : months.map((month) => <article key={month.id} style={cardStyle}><div style={cardHeaderStyle}><div><strong>{month.sourceMonth} 月版班表</strong><p style={subtleStyle}>版本 {month.versionNo} · {statusLabel[month.status]} · {month.changeReason}</p></div>{canEdit && month.status === "draft" ? <button disabled={Boolean(busy)} onClick={() => void act(month.id, "publish")} style={primaryButtonStyle} type="button">{busy === month.id ? "處理中..." : "發布本月"}</button> : null}{canEdit && month.status === "published" ? <button disabled={Boolean(busy)} onClick={() => void act(month.id, "disable")} style={secondaryButtonStyle} type="button">{busy === month.id ? "處理中..." : "停用"}</button> : null}</div><div style={assetGridStyle}>{month.assets.map((asset) => <a href={asset.originalContentUrl} key={asset.branch} rel="noreferrer" style={assetStyle} target="_blank"><img alt={`${month.sourceMonth} ${asset.branch}班表`} src={asset.previewImageUrl} style={assetPreviewStyle} /><span>{asset.branch}</span></a>)}</div></article>)}</div></section>
  </section></main>;
}
