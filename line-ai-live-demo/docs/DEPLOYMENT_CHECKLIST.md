...
- [ ] 若部署門診班表管理，`npm run validate:schedule-month` 通過，且 `20260718_schedule_month_assets.sql` 已在客戶 Supabase SQL Editor 成功執行。
...
- [ ] 若啟用真人接手摘要 email：已驗證寄件網域並在 Vercel Production 設定 `RESEND_API_KEY`、`HANDOFF_DIGEST_FROM`；各館只填自己的 `HANDOFF_DIGEST_<BRANCH>_TO`。未填收件人的館別不寄信。
...
- [ ] 預約可收集療程、館別、三個時段、初診/複診、姓名、電話；可改期與取消。
...
- [ ] owner／manager 上傳同月四館班表圖與結構化 CSV，確認自動縮圖、草稿、發布、停用與版本歷史；已發布當月可傳圖，未發布或舊月份一律不傳舊圖並轉真人。

## 報表、通知與交接

- [ ] 手動 dry-run：`npm run reporting:daily -- --dry-run --date=<前一天日期>`。
- [ ] 每日排程已在 Vercel 生效，台灣時間凌晨 01:00 統計前一天。
- [ ] 固定時段真人接手摘要採台灣時間：週一至週五 20:00、週六 16:00、週日 16:00。Vercel Hobby 不支援這種同日多時段商業排程；正式啟用前需使用 Vercel Pro 或等效外部排程，並以 `/api/cron/handoff-digest` 搭配 `CRON_SECRET` 呼叫。
- [ ] 診所管理者已在 `/admin/team` 寄送高雄館測試信確認寄達；正式摘要只含館別與待處理數量，不含姓名、電話、LINE ID 或原始對話。
